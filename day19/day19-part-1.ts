...
const countBeacons = (scanners: ScannerData[]): number => {
  const beacons = new Set<string>();
  const translated = new Set([0]);
  const queue = [scanners[0]];
  addPositionsToSet(scanners[0].positions, beacons);
  ...
  while (queue.length) {
    ...
    for (const overlap of overlappingScanners) {
      ...
      addPositionsToSet(overlappingScanner.positions, beacons);
...
const addPositionsToSet = (positions: Position[], set: Set<string>): void => {
...
const main: DayFunction = async (inputPath) => {
  ...
  const totalBeacons = countBeacons(scannerData);
