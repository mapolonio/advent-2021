...
export const getMoves = (maze: string[], amphipod: Amphipod): Move[] => {
  ...
  if (col === finalCol) {
    // Already in destination at bottom OR partner is already in the bottom
    if (row === 3 || (row === 2 && maze[3].charAt(finalCol) === label)) {
      return [];
    }
  }
  ...
  while (queue.length) {
    ...
    if (destination.col === finalCol) {
      // Room has a fellow amphipod
      if (destination.row === 2 && maze[3].charAt(destination.col) === label) {
        moves.push({
          amphipod,
          destination
        });
