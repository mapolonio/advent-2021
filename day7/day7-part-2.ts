...
const calculateFuel = (positions: Array<number>, target: number) => {
  let fuel = 0;
  const fuelCalculator = getFuelCalculator();

  for (const position of positions) {
    fuel += fuelCalculator(Math.abs(position - target));
  }
...
const getFuelCalculator = () => {
  const fuelCostCache: { [index: string]: number } = {};

  return (distance: number): number => {
    const key = distance.toString();
    if (key in fuelCostCache) {
      return fuelCostCache[key];
    } else {
      const result = getSumOfN(distance);

      fuelCostCache[distance] = result;
